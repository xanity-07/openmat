...
func ErrCode(err error) Code {
	if pgerr, ok := errors.AsType[*Error](err); ok {
		return pgerr.Code
	}
	return Other
}
...
func HandleError(err error) error {
	...
	if _, ok := errors.AsType[*errs.AppError](err); ok {
		return err
	}
	...
	if pgerr, ok := errors.AsType[*pgconn.PgError](err); ok {
		...
		switch sqlErr.Code {
		case ForeignKeyViolation:
			return errs.NewBadRequestError(userMessage, &errorCode, nil, nil)
		...
		case NotNullViolation:
			...
			return errs.NewBadRequestError(userMessage, &errorCode, fieldErrors, nil)
	...
	case errors.Is(err, pgx.ErrNoRows), errors.Is(err, sql.ErrNoRows):
		...
		if strings.Contains(errMsg, tablePrefix) {
			table, _, _ := strings.Cut(strings.Split(errMsg, tablePrefix)[1], ":")
			entityName := getEntityName(table, "")
			return errs.NewNotFoundError(fmt.Sprintf("%s not found", entityName), nil)
		}
